...
function Vediocard({video,setDeleteVideoResponse,insideCategory}) {
  ...
  return (
    <div>
       <Card style={{ width: '18rem' }} className='m-5 p-2' draggable onDragStart={e=>dragStart(e,video?.id)}>
      <Card.Img variant="top" src={video.url} onClick={handleShow} />
      <Card.Body>
        <Card.Title className=' text-center' >
          <h3>{video.title}</h3>
          {insideCategory?null:<button className='btn btn-link' onClick={e=>removeVideo(video?.id)}><i className="fa-solid fa-trash fa-xl" style={{color:"red"}}></i> </button>}
        </Card.Title>
      </Card.Body>
    </Card>
